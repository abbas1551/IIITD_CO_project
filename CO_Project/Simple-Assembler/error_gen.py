...
for i in final_data:    
        ...
        if 'hlt' in i:  #checks for more than one halt
                    hlt_count += 1
                    if hlt_count > 1:
                        print("hlt occurs more than once")
                        error_present = True######checks out till here
        if i[0] == 'var':
        ...
        if (i[0] == 'mov'):
            ...
            if (len(i) == 3):
                if i[2][0] == '$':
                    if (i[2][1:].isdigit() == False):
                        print("The value given is not a pure integer or it is a negative value")
                        error_present = True
                    else:
                        if (int(i[2][1:]) > 127):
                            print(i[2][1:],"value overflow, greater than 127")
